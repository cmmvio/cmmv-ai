...
import { TongyiEmbedding } from '../../src/embeddings/tongyi.embedding';
import { AbstractEmbedding } from '../../src/embeddings/embedding.abstract';

vi.mock('@langchain/community/embeddings/alibaba_tongyi', () => ({
    AlibabaTongyiEmbeddings: vi.fn().mockImplementation(() => ({
...
vi.mock('@cmmv/core', async () => {
    ...
    return {
        ...
        Config: {
            get: vi.fn(() => 'your-api-key'),
        },
    };
});

describe('TongyiEmbedding', () => {
    let embedding: TongyiEmbedding;

    beforeEach(() => {
        vi.clearAllMocks();
        embedding = new TongyiEmbedding();
    });
