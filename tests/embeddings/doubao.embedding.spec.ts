...
import { DoubaoEmbedding } from '../../src/embeddings/doubao.embedding';
    ...
    return {
        ...
        Config: {
            get: vi.fn((key: string) => {
                if (key === 'ai.tokenizer.apiKey') return 'test-api-key';
                return null;
            }),
        },
    };
...
vi.mock('@langchain/community/embeddings/bytedance_doubao', () => ({
    ByteDanceDoubaoEmbeddings: vi.fn().mockImplementation(() => ({
...
describe('DoubaoEmbedding', () => {
    let embedding: DoubaoEmbedding;

    beforeEach(() => {
        vi.clearAllMocks();
        embedding = new DoubaoEmbedding();
    });
