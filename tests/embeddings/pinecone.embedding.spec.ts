...
import { PineconeEmbedding } from '../../src/embeddings/pinecone.embedding';
import { AbstractEmbedding } from '../../src/embeddings/embedding.abstract';

vi.mock('@langchain/pinecone', () => ({
    PineconeEmbeddings: vi.fn().mockImplementation(() => ({
...
vi.mock('@cmmv/core', async () => {
    ...
    return {
        ...
        Config: {
            get: vi.fn(() => 'multilingual-e5-large'),
        },
    };
});

describe('PineconeEmbedding', () => {
    let embedding: PineconeEmbedding;

    beforeEach(() => {
        vi.clearAllMocks();
        embedding = new PineconeEmbedding();
    });
