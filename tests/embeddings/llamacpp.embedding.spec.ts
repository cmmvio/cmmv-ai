...
import { LlamaCppEmbedding } from '../../src/embeddings/llamacpp.embedding';
import { AbstractEmbedding } from '../../src/embeddings/embedding.abstract';

vi.mock('@langchain/community/embeddings/llama_cpp', () => ({
    LlamaCppEmbeddings: vi.fn().mockImplementation(() => ({
...
vi.mock('@cmmv/core', async () => {
    ...
    return {
        ...
        Config: {
            get: vi.fn(() => './models/llama-2-7b.gguf'),
        },
    };
});

describe('LlamaCppEmbedding', () => {
    let embedding: LlamaCppEmbedding;

    beforeEach(() => {
        vi.clearAllMocks();
        embedding = new LlamaCppEmbedding();
    });
