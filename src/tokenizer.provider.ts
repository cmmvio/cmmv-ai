...
export class Tokenizer extends EventEmitter {
    ...
    async initialize() {
        ...
        const chunkOverlap = Config.get<number>(
        ...
        const splitterMarkdown = RecursiveCharacterTextSplitter.fromLanguage(
            'markdown',
            {
                chunkSize,
                chunkOverlap,
        ...
        for (const filename of files) {
            ...
            else output = await splitterJs.createDocuments([content]);
        ...
        return Promise.all(
            output.map(async (document) => ({
                ...
                vector: new Float32Array(
                    await this.embedder.embedQuery(document.pageContent),
                ),
            })),
        );
    }

    private async processDocuments<T>(
        output: any[],
        filename: string,
    ): Promise<void[]> {
        return Promise.all(
            output.map(
                async (document) =>
                    await this.processEntity(filename, document),
            ),
        );
    }

    private async processEntity(filename: string, document: any) {
        try {
